...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const endElement = useWindowScroll
      ? document.querySelector(`.${styles.scrollStackEnd}`)
      : scrollerRef.current?.querySelector(`.${styles.scrollStackEnd}`);
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      const lenis = new Lenis({
        wrapper: scroller,
        content: scroller.querySelector(`.${styles.scrollStackInner}`),
        duration: 1.2,
        easing: t => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 2,
